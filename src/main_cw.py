...
def sx1278_init(air_frequency):
    ...
    write_reg_bit(REG_PA_CONFIG, 3, 0)  # 设置输出功率衰减为 0 dBm (17dBm)
    write_reg_bit(REG_PA_CONFIG, 2, 0)  # 设置输出功率衰减为 0 dBm (17dBm)
    write_reg_bit(REG_PA_CONFIG, 1, 0)  # 设置输出功率衰减为 0 dBm (17dBm)
    write_reg_bit(REG_PA_CONFIG, 0, 0)  # 设置输出功率衰减为 0 dBm (17dBm)
...
while True:
    sx1278_init(air_frequency=145695000)
    content = input("输入要发送的内容: ")
    sx1278_send(content)

# Example CQ CQ CQ BG7ZDQ BG7ZDQ BG7ZDQ K